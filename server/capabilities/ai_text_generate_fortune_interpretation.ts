...
export const callCapabilities = async (input: InputParams): Promise<Response> => {
  try {
    ...
    const prompt = `你是一位精通易经与东方古典文化的隐世占卜师，擅长深度解读签文。请对以下签文进行深度解读。

签文内容：${input.fortune_text}

请按照以下四个部分进行解读，不要使用任何 markdown 符号（如*#等），不要重复标题：

时节呼应：分析当前时节与签文的关联，反映用户内心的季节感受和情感波动

卦象智慧：解读易经卦象的深层含义和智慧启示

具体指引：针对用户当前具体情况，给出具体的关注和调整方向

行动建议：提供具体的行动建议和心态调整方向

要求：
1. 每个部分直接以"时节呼应""卦象智慧""具体指引""行动建议"开头，后面跟冒号
